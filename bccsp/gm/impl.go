...
func New(securityLevel int, hashFamily string, keyStore bccsp.KeyStore) (bccsp.BCCSP, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("Failed initializing configuration at [%v,%v]: %w", securityLevel, hashFamily, err)
	}
...
func (csp *impl) KeyGen(opts bccsp.KeyGenOpts) (k bccsp.Key, err error) {
	...
	if !opts.Ephemeral() {
		...
		if err != nil {
			return nil, fmt.Errorf("Failed storing key [%s]: %w", opts.Algorithm(), err)
		}
	}
...
func (csp *impl) Hash(msg []byte, opts bccsp.HashOpts) (digest []byte, err error) {
	...
	if err != nil {
		return nil, fmt.Errorf("Failed hashing with opts [%v]: %w", opts, err)
	}
...
func (csp *impl) GetHash(opts bccsp.HashOpts) (h hash.Hash, err error) {
	...
	if err != nil {
		return nil, fmt.Errorf("Failed getting hash function with opts [%v]: %w", opts, err)
	}
...
func (csp *impl) Verify(k bccsp.Key, signature, digest []byte, opts bccsp.SignerOpts) (valid bool, err error) {
	...
	if !found {
		return false, fmt.Errorf("Unsupported 'VerifyKey' provided [%T]", k)
	}
...
func (csp *impl) Decrypt(k bccsp.Key, ciphertext []byte, opts bccsp.DecrypterOpts) (plaintext []byte, err error) {
	...
	if err != nil {
		return nil, fmt.Errorf("Failed decrypting with opts [%v]: %w", opts, err)
	}
