...
func (ki *x509PublicKeyImportOptsKeyImporter) KeyImport(raw interface{}, opts bccsp.KeyImportOpts) (k bccsp.Key, err error) {
	...
	case []byte:
		...
		// Prefer GM certificates, but allow standard x509 for compatibility.
		if cert, sm2Err := sm2.ReadCertificateFromMem(r); sm2Err == nil {
			publicKey = cert.PublicKey
		} else {
			x509Cert, parseErr := x509.ParseCertificate(r)
			if parseErr != nil {
				return nil, fmt.Errorf("failed to parse certificate: %v", parseErr)
			}
	...
	case *x509.Certificate:
		publicKey = r.PublicKey
	default:
		return nil, errors.New("Invalid raw material. Expected *x509.Certificate, *sm2.Certificate or DER bytes.")
	}

	switch pkTyped := publicKey.(type) {
	case sm2.PublicKey:
		fmt.Printf("bccsp gm keyimport pk is sm2.PublicKey")
		der, err := sm2.MarshalSm2PublicKey(&pkTyped)
		if err != nil {
